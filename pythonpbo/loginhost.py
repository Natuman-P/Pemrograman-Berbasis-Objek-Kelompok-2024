...
class Login_Window:
    def __init__(self, root):
        ...
        # Login button
        loginbtn = Button(frame, command=self.open_pagehost_window, text="Login", font=("times new roman", 15, "bold"), bd=3, relief=RIDGE, fg="white", bg="red", activeforeground="white", activebackground="red")
    ...
    def open_pagehost_window(self):
        try:
            register_path = os.path.join(r"C:\Users\Pongo\Pemrograman-Berbasis-Objek-Kelompok-2024\pythonpbo", "host.py")
            subprocess.Popen(["python", register_path])
        except Exception as e:
            messagebox.showerror("Error", f"Failed to open register window: {e}")
